...
export class ProfileController implements IProfileController {
  ...
  async findUserByTheirId(call: any, callback: any): Promise<void> {
    ...
    try {
      const response = await this.userRepository.findUserById(id);
      if (!response) {
  ...
  async editProfile(call: any, callback: any): Promise<void> {
    ...
    try {
      ...
      const response = await this.userRepository.updateUser(id, { name, phone });
      if (!response) {
  ...
  async addNewAddress(call: any, callback: any): Promise<void> {
    ...
    try {
      let updatedAddress;
      if (index === -1) {
        updatedAddress = await this.userRepository.addAddress(userId, address);
      } else {
        updatedAddress = await this.userRepository.updateAddress(userId, index, address);
      }
      callback(null, {
        message: 'Success',
        address: {
  ...
  async deleteUserAddress(call: any, callback: any): Promise<void> {
    ...
    try {
      const response = await this.userRepository.deleteAddressByIndex(id, index);
